...
def insert_user ( name: str, passwd, full_name: str, email: str ):
    ...
    if not check_username_is_available ( name ):
        raise ValueError ( "The chosen user name is already taken, please choose a different one." )

    conn = sqlite3.connect ( DB_PATH )
    cursor conn.cursor()
    cursor = conn.cursor()
    cursor.execute ( "INSERT INTO user_info (name, passwd, full_name, email) VALUES (?, ?, ?, ?)", ( name, passwd, full_name, email ), )
    conn.commit()
    conn.close()
...
def get_user_info ( name: str ):
    ...
    conn = sqlite3.connect ( DB_PATH )
    cursor = conn.cursor()
    cursor.execute ( "SELECT name, passwd, full_name, email, disable FROM user_info WHERE name = ?", ( name, ), )
...
def insert_post ( post_name: str, url: str, username: str, status ):
    ...
    if not check_admin_code_is_available ( admin_url ):
        raise ValueError ( "The chosen post name is already taken, Please choose a different post name." )

    conn = sqlite3.connect ( DB_PATH )
    cursor = conn.cursor()
    cursor.execute ( "INSERT INTO post_info (post_name, url, username, status) VALUES (?, ?, ?, ?)", ( post_name, url, username, status), )
    conn.commit()
    conn.close()
...
def update_status ( post_name: str, status: str ):
    ...
    conn - sqlite3.connect ( DB_PATH )
    cursor = conn.cursor()
    cursor.execute ( "UPDATE post_info SET status = ? WHERE post_name = ?", (status, post_name) )
    conn.commit()
    conn.close()
